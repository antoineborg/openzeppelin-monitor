...
### Example 4: Monitor Program Upgrades (Solana)

This example monitors program upgrades on Solana mainnet.
...
Set up the token instruction monitor:

```bash
# Only necessary if you haven't already run the automated setup script (Option 1: Automated Setup)
cp examples/config/monitors/solana_upgrade.json config/monitors/solana_upgrade.json
```
...
```json
{
  ...
  "match_conditions": {
    ...
    "events": [
      {
        "signature": "Upgraded program <YOUR_SOLANA_PROGRAM_ADDRESS>",
        "expression": null
      }
    ],
    "transactions": [
...
* Solana monitors track program instructions and events
* Replace `YOUR_SOLANA_PROGRAM_ADDRESS` with the actual program address you want to monitor
...
Set up Slack notifications for Solana events:
...
```json
{
  "solana_upgrade_slack": {
    "name": "Program Upgrade Slack Notification",
    "trigger_type": "slack",
    "config": {
      "slack_url": {
        "type": "plain",
      ...
      "message": {
        "title": "solana_upgrade_slack triggered",
        "body": "${monitor.name} triggered because of a program upgrade on this transaction: https://solscan.io/tx/${transaction.signature}"
      }
    }
  }
}
...
1. Check for new Solana blocks every minute
2. Watch for token instructions on the monitored program
3. Send notifications via Slack when events occur
